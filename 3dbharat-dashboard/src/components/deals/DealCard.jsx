...
export function DealCard({ deal }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5 flex flex-col gap-4 hover:border-accent/50 transition-colors">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <Link href={`/deals/${deal.id}`} className="font-display font-semibold text-sm hover:text-accent">
            {deal.name}
          </Link>
      ...
      <Link
        href={`/deals/${deal.id}`}
        className="text-center text-sm font-medium py-2 rounded-lg border border-border hover:border-accent hover:text-accent transition-colors"
      >
        View details
      </Link>
    </div>
